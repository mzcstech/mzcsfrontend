...
class AddTemplate extends React.Component {
    ...
    handleSubmit = (event) => {
        if (this.state.companyName != '') {
            var templateVo = {
                generalContractId:this.state.generalContractId,
    ...
    findById = () => {
        ...
        fetch(SERVER_URL + '/generalContract/findById/' + generalContractId,
            ...
            .then((responseData) => {
                this.setState({
                    generalContractId:responseData.data.generalContractId,
        ...
        return (
            <div>
              <Dialog open={this.state.openDialog} fullWidth={this.state.fullWidth} 
                ...
                <div>
                    <Button variant="contained" color="primary" style={{ 'margin': '10px',backgroundColor:'#2A608F' }} onClick={this.showAdd}>编辑</Button>
                </div>
                <Snackbar

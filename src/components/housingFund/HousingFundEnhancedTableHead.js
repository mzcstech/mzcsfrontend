...
require('./styles/HousingFundEnhancedTableHead.css')
//第一列的标题
{/*
...
const rows = [
    ...
    { id: 'saler', numeric: true, disablePadding: false, label: '签单人' },    
    {
        ...
        label: '查看',
        Cell: ({ row }) =>
            (<ViewHousingFund fetchTemplate={this.fetchTemplate} templeteId={row.housingFundId} ></ViewHousingFund>)
    },
    {
        id: 'updatePagebutton',
        numeric: false,
        ...
        Cell: ({ row }) =>
            (<EditHousingFund editTemplate={this.editTemplate} fetchTemplate={this.fetchTemplate} templeteId={row.housingFundId} />)
    },
    {
        id: 'delbutton',
        numeric: false,
        ...
        label: '删除',
        width: 70,
        Cell: ({ row }) => (<Button size="small" variant="text" color="primary" onClick={() => { this.confirmDelete(row.HousingFundId) }}>删除</Button>)
    }
];
